...
```
sync-all-cron.sh          ← launchdから呼ばれる唯一のエントリポイント
  ├── sync-meetings-cron.sh    (circleback → meetings/)
  ├── sync-paulo-cron.sh       (gmail → output/paulo-newsletter/)
  └── sync-market-hack-cron.sh (gmail → output/market-hack-magazine/)
...
- **実行時間**: 毎晩 1:00
- **機能**: プロセスロック（`/tmp/sec9-works-sync-all.lock`）で重複実行を防止

### sync-meetings-cron.sh
- **目的**: Sec9社（circleback）の議事録を自動同期
- **出力先**: `meetings/`

### sync-paulo-cron.sh
- **目的**: Pauloニュースレターの記事を自動取得
- **出力先**: `output/paulo-newsletter/`

### sync-market-hack-cron.sh
- **目的**: Market Hack Magazineの記事を自動取得
- **出力先**: `output/market-hack-magazine/`

### git-sync-helper.sh（共通ヘルパー）
- **目的**: git操作の排他制御・安全なcommit & push
- **機能**:
  - ロックファイル（`/tmp/sec9-works-git.lock`）で排他制御
...
# 状態確認
launchctl list | grep sec9.sync-all

# 手動登録（初回・再起動後など）
launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.sec9.sync-all.plist

# 登録解除
launchctl bootout gui/$(id -u) ~/Library/LaunchAgents/com.sec9.sync-all.plist
```
...
```bash
rm -f /tmp/sec9-works-git.lock /tmp/sec9-works-sync-all.lock
```
